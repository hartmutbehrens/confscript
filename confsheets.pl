...
foreach my $omc (keys %gpu) {
	foreach my $mfsName (keys %{$gpu{$omc}}) {
		...
		print FILE Tr(th(["BSC NAME","GPU's","Ater-PS", "NSEI's","NSVC's per NSEI","Gb Bandwidth"])),"\n";
		foreach my $bscName (sort keys %{$gpu{$omc}{$mfsName}}) {
			print FILE Tr(td([$bscName,@{$gpu{$omc}{$mfsName}{$bscName}}{qw/GPUS ATERPS NSEIS NSVCS Gb/}])),"\n";	
		}
		print FILE end_table;
		close FILE;
	}
